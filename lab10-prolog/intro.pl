...
myConcat([], L, L).
myConcat([H|T], L, [H|R]):- myConcat(T, L, R).
...
myReverse([], []).
myReverse([H|T], Res):- myReverse(T, R), myConcat(R, [H], Res).
...
factorial(0, 1).
factorial(1, 1).
factorial(N, F):- N > 0, NN is N - 1, factorial(NN, FF), F is FF * N.
check4:-
	tests([
...
descendantOf(X, Y):- arc(Y, X); (arc(DescOfY, X), descendantOf(DescOfY, Y)).
...
sameTree(N1, N2):- descendantOf(N1, N2); descendantOf(N2, N1); (descendantOf(N1, X), descendantOf(N2, X)).
...
drum(N, N, [N]).
drum(N1, N2, [N1, N2]):- arc(N1, N2); arc(N2, N1).
...
cost(N1, N2, C):- arc(N1, X), descendantOf(N2, X), cost(X, N2, N), C is N + 1.
cost(N1, N2, C):- arc(X, N1), cost(X, N2, N), C is N - 1.
