...
(define (no-base n Bases)
  (if (null? Bases)
      0
      (+ (boolean->integer (palindrome? (num->base n (car Bases)))) (no-base n (cdr Bases)))))

(define (first-b-pal start b)
  (if (>= (no-base start (list 2 3 4 5 6 7 8 9 10)) b)
      start
...
(define (best-length Digits len res)
   (if (< len (length res))
       res
       (if (and (palindrome? (take Digits len)) (> len (length res)))
           (best-length (cdr Digits) (length (cdr Digits)) (take Digits len))
           (best-length (cdr Digits) (length (cdr Digits)) (best-length Digits (- len 1) res)))))

(define (longest-palindrome n)
   (list->num (best-length (num->base n 10) (length (num->base n 10)) null)))
