...
computeLength getLineSegment getStartPoint getEndPoint = let segment = getLineSegment
...
computeLength2 :: ((Double, Double), (Double, Double)) 
            -> (((Double, Double), (Double, Double)) -> (Double, Double)) 
            -> (((Double, Double), (Double, Double)) -> (Double, Double)) -> Double
computeLength2 getLineSegment getStartPoint getEndPoint = sqrt $ (x1 - x2) ** 2 + (y1 - y2) ** 2 where
...
mergeSort :: [Int] -> [Int]
mergeSort lst = let computeHalf f lst = f (quot (length lst) 2) lst
...
mergeSort2 lst = case lst of
    [] -> []
    [x] -> [x]
    _ -> merge (mergeSort $ fstHalf lst) (mergeSort $ sndHalf lst) where
...
playerWins :: Int -> Bool
playerWins candies = let player candies = candies > 0 && (not (opponent $ candies - 1) || not (opponent $ candies - 2))
                         opponent candies = candies > 1 && (not (player $ candies - 2) || not (player $ candies - 3))
                         in player candies

playerWins2 :: Int -> Bool
playerWins2 candies = player candies where
