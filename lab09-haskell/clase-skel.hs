...
instance Eq a => Eq (BST a) where
    (==) BSTNil BSTNil = True
    (==) tree1@(BSTNod k1 l1 r1) tree2@(BSTNod k2 l2 r2) = k1 == k2 && (==) l1 l2 && (==) r1 r2
    (==) _ _ = False
...
printLevel tab level (BSTNod k l r) = replicate level tab ++ show k ++ "\n" ++
    printLevel tab (level + 1) l ++
    printLevel tab (level + 1) r
...
instance Invertible (BST a) where
    ...
    invert tree@(BSTNod k l r) = BSTNod k (invert r) (invert l)
check4 :: TestData
check4 = tests_ 4 $
    [
...
instance Functor BST where
    ...
    fmap f (BSTNod k l r) = BSTNod (f k) (fmap f l) (fmap f r)
...
instance Foldable BST where
    foldr _ acc BSTNil = acc
    foldr f acc (BSTNod k l r) = foldr f (f k (foldr f acc r)) l
