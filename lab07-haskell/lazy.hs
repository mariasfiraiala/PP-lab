...
findStringsLongerThanTenChars l = filter ((>= 10) . length) l
...
newtonRaphson x g dg = x : newtonRaphson (x - (g x / dg x)) g dg
...
newtonRaphsonSolve x_old atol g dg = let nr = newtonRaphson x_old g dg
                                         nr_d = tail nr
                                         dif = zipWith (\ x y -> (x, abs $ x - y)) nr_d nr
                                         res = filter (\(x, y) -> y <= atol) dif
                                         in fst $ head res
