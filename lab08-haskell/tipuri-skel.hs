...
data BST a = BSTNode {l::BST a, r::BST a, k::a} | BSTNil deriving Show

insertElem :: (Ord a, Eq a) => BST a -> a -> BST a
insertElem BSTNil a = BSTNode BSTNil BSTNil a
insertElem tree@(BSTNode l r k) a
  | a <= k = tree { l =insertElem l a}
  | otherwise = tree { r = insertElem r a}

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing
findElem tree@(BSTNode l r k) a
  | a == k = Just a
  | a < k = findElem l a
...
inorder :: BST a -> [a]
inorder BSTNil = []
inorder tree@(BSTNode l r k) = inorder l ++ [k] ++ inorder r

size :: BST a -> Int
size BSTNil = 0
size tree@(BSTNode l r k) = 1 + size l + size r

height :: BST a -> Int
height BSTNil = 0
height tree@(BSTNode l r k) = max (height l + 1) (height r + 1)

isBalanced :: (Ord a, Num a) => BST a -> Bool
isBalanced BSTNil = True
isBalanced tree@(BSTNode l r k) = abs (height l - height r) <= 1 && isBalanced l && isBalanced r
...
data Tree a = TreeNode{children::[Tree a], key::a} deriving Show
...
consElem a (Atom x) = List [Atom a, Atom x]
consElem a (List xs) = List (Atom a : xs)

consList :: NestedList a -> NestedList a -> NestedList a
consList a (Atom x) = List [a, Atom x]
consList a (List xs) = List (a : xs)

headList :: NestedList a -> NestedList a
headList (Atom x) = error "head undefined on atoms"
headList (List xs) = head xs

tailList :: NestedList a -> NestedList a
tailList (Atom x) = error "tail undefined on atoms"
tailList (List xs) = List (tail xs)

deepEqual :: Eq a => NestedList a -> NestedList a -> Bool
deepEqual (Atom x) (Atom y) = x == y
deepEqual (List xs) (List ys) = and (zipWith deepEqual xs ys) && length xs == length ys
deepEqual _ _ = False
